...
function load_graph(list) {
	var object_list = list.collated_emotions; 
	console.log(JSON.stringify(object_list));
	for (var i = 0; i < object_list.length; i++) {
...
function graphQueryProcessor(graph_query_response){
	...
	for (x in graph_query_response['collated_emotions'][0]){
		...
		joy_series.push([Date.UTC(year,month,day),value])
	...
	for (x in graph_query_response['collated_emotions'][1]){
		...
		console.log('Year:',year, 'Month', month, 'Day:', day, 'Value:', value);
		anger_series.push([Date.UTC(year,month,day),value])
	...
	for (x in graph_query_response['collated_emotions'][2]){
		...
		console.log('Year:',year, 'Month', month, 'Day:', day, 'Value:', value);
		sadness_series.push([Date.UTC(year,month,day),value])
	...
	for (x in graph_query_response['collated_emotions'][3]){
		...
		console.log('Year:',year, 'Month', month, 'Day:', day, 'Value:', value);
		disgust_series.push([Date.UTC(year,month,day),value])
	...
	for (x in graph_query_response['collated_emotions'][4]){
		...
		console.log('Year:',year, 'Month', month, 'Day:', day, 'Value:', value);
		fear_series.push([Date.UTC(year,month,day),value])
...
$(document).ready(function(){
    ...
    //console.log('Printing first', data_series[1]);
    //graphRenderer(data_series, data_series, data_series, data_series, data_series,data_series);

    //data_series2 = [10,12,7,9];
   	//name2 = 'Blue';
    //graphRenderer(data_series2, name2);
